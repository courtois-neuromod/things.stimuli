...
The current image dataset (which includes >4k images representing 720 image concepts) is a subset of the THINGS images, a freely available dataset of 26k images of 1,854 object concepts curated systematically from everyday American English language by the THINGS Initiative (https://things-initiative.org/).

The THINGS images are saved under ``images_fmri.zip``. The copyright of each image belongs to the owner(s) of the image. Images are made available strictly for research and educational purposes under the principle of fair use. Commercial use, reproduction, distribution, or modification of the image is not permitted without the explicit consent of the owner(s).
...
Note that the same password is used for the ``images_test_fmri.zip`` and ``images_catch_fmri.zip`` files.
